...
print(result)
# FitResult(model=chapman_richards, n=8, R²=0.998, RMSE=0.31, AICc=-12.4)

# Select the best model from all 12 forms
best, ranked = select_model(dbh, height)
print(ranked[["model", "aicc", "r2"]].head())
```
...
stand = Stand(trees=trees, plot_area_ha=0.1)
...
```python
from pylometree.io import stand_from_csv

stand = stand_from_csv(
    "survey_plot.csv",
    plot_area_ha=0.25,
    dbh_col="DBH_cm",
    height_col="H_m",
    wood_density_col="rho",
    species_col="species",
)
print(f"n = {len(stand)}, AGB = {stand.agb_mg_ha():.2f} Mg/ha")
```
